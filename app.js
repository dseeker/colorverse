 ...
 * DJB2 algorithm: good distribution, fast execution, minimal collisions
 */
...
/**
 * Generates consistent seed for items WITH category context
 * Combines categoryKey + itemKey for uniqueness
 * Used for: category grids, item detail pages, related items
 */
...
/**
 * Generates seed for items WITHOUT category context
 * Used for: homepage categories, seasonal gallery, standalone items
 */
...
function createImagePreview() {
  ...
  if (!previewOverlay) {
    ...
    previewOverlay.className =
      "fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 opacity-0 pointer-events-none transition-opacity duration-300";
    ...
    elementTimeout = setTimeout(() => {
      ...
      if (isHovering) {
        ...
        const hasErrorIndicator =
          parentContainer && parentContainer.querySelector(".image-error-indicator");
  ...
  document.querySelectorAll("img[data-src]").forEach(img => {
    ...
    if (wrapper) {
      ...
      if (!loader) {
        ...
        loader.className =
          "image-loading-indicator absolute inset-0 flex items-center justify-center bg-gray-100 bg-opacity-50 z-0";
...
// --- LRU Cache for Image URLs (max 100 entries) ---
...
function renderDailyPick(data) {
  ...
  const categoryKey = data.seasonal_gallery ? "seasonal" : Object.keys(data.categories)[0];
...
function renderDailyPickPage(dailyPickData) {
  ...
  const pickItem = dailyPick.item || {
    ...
    description:
      "A serene garden with mystical creatures, magical flowers, and hidden pathways through an enchanted landscape",
  };
...
function renderCategory(categoryData, categoryKey, currentPage = 1, sortBy = "popular") {
  ...
  const itemsForPage = generateCategoryPageItems(
    categoryData,
    categoryKey,
    currentPage,
    itemsPerPage,
...
function generateCategoryPageItems(
  categoryData,
  categoryKey,
  page,
  itemsPerPage,
    ...
    try {
      ...
      if (attempt === maxRetries) {
        ...
        if (typeof showToast === "function") {
          showToast(
            `Failed to load image after ${maxRetries} attempts. Please try again later.`,
            "error"
          );
        }
...
function handleRouteChange() {
  ...
  if (window.imageLoadQueue) {
    ...
    if (canceledCount > 0) {
      console.log(
        `[Navigation] Canceled ${canceledCount} pending image downloads due to page change`
      );
      showToast(`Canceled ${canceledCount} pending downloads`, "info", 1500);
    }
  }
    ...
    try {
      if (hash === "#" || hash === "#/") {
        ...
        if (categoryKey === "seasonal" && siteData.seasonal_gallery) {
          ...
          mainContent.innerHTML = renderCategory(
            seasonalCategory,
            categoryKey,
            currentPage,
            sortBy
      ...
      mainContent.innerHTML =
        '<p class="text-center text-red-500">An error occurred while loading this page. Please try again later.</p>';
      mainContent.classList.remove("hidden");
    } finally {
      showLoading(false);
...
function shouldRefreshCache() {
  ...
  try {
    ...
    if (categoryCount < 25) {
      console.log(
        `Cached data has only ${categoryCount} categories, forcing refresh to get all 25`
      );
      return true;
    }
...
async function loadCategoriesInBackground() {
  ...
  const CONCURRENCY_LIMIT = 2; // Conservative parallel loading (Q3-A)
  ...
  for (let i = 0; i < categoryList.length; i += CONCURRENCY_LIMIT) {
    ...
    try {
      ...
      await Promise.all(
        batch.map(async categoryKey => {
          try {
            ...
            console.log(
              `✅ ${categoryKey} loaded (${loadedCount}/${totalCategories} - ${progress}%)`
            );

            if (loadedCount % 5 === 0) {
              showToast(
                `Loaded ${loadedCount}/${totalCategories} categories (${progress}%)`,
                "info",
                2000
              );
            }
          } catch (error) {
            console.error(`❌ Failed to load ${categoryKey}:`, error);
          }
        })
      );
      ...
      console.error(`❌ Batch ${Math.ceil(i / CONCURRENCY_LIMIT) + 1} failed:`, error);
...
function getCategoryInfo(categoryKey) {
  const categoryMap = {
    animals: {
      ...
      specialInstructions:
        "Include domestic pets, wild animals, farm animals, zoo animals, and marine life.",
    },
    fantasy: {
      title: "Fantasy & Magic",
      ...
      specialInstructions:
        "Include dragons, unicorns, fairies, wizards, magical castles, and mythical creatures.",
    },
    mandalas: {
      ...
      specialInstructions:
        "Include geometric mandalas, floral mandalas, sacred geometry, and meditation patterns.",
    },
    vehicles: {
      ...
      specialInstructions:
        "Include cars, trucks, airplanes, boats, trains, motorcycles, and emergency vehicles.",
    },
    nature: {
      ...
      specialInstructions:
        "Include forests, mountains, beaches, gardens, trees, and natural landscapes.",
    },
    food: {
      ...
      specialInstructions:
        "Include fruits, vegetables, desserts, cooking scenes, and international cuisine.",
    },
    space: {
      ...
      specialInstructions:
        "Include planets, rockets, astronauts, space stations, and cosmic scenes.",
    },
    abstract: {
      title: "Abstract Art",
      ...
      specialInstructions:
        "Include geometric patterns, abstract shapes, artistic designs, and creative compositions.",
    },
    flowers: {
      ...
      specialInstructions:
        "Include roses, sunflowers, daisies, bouquets, and botanical illustrations.",
    },
    ocean: {
      ...
      specialInstructions:
        "Include fish, dolphins, coral reefs, underwater scenes, and marine life.",
    },
    dinosaurs: {
      ...
      specialInstructions:
        "Include T-Rex, Triceratops, Stegosaurus, and various dinosaur species in prehistoric settings.",
    },
    mythical: {
      ...
      specialInstructions:
        "Include griffins, phoenixes, centaurs, and creatures from various mythologies.",
    },
    birds: {
      title: "Birds & Flight",
    ...
    architecture: {
      ...
      specialInstructions:
        "Include castles, cathedrals, modern buildings, and architectural landmarks.",
    },
    sports: {
      ...
      specialInstructions:
        "Include soccer, basketball, swimming, and various recreational activities.",
    },
    memes: {
      ...
      specialInstructions:
        "Include popular meme formats, viral animals, and internet culture references suitable for coloring.",
    },
    adult_zen: {
      ...
      specialInstructions:
        "Include intricate patterns, wine themes, coffee art, and sophisticated motifs.",
    },
    spicy_bold: {
      ...
      specialInstructions:
        "Include skull art, tattoo designs, rock band themes, and edgy patterns.",
    },
    children_characters: {
      ...
      specialInstructions:
        "Include popular children's cartoon characters, fairy tale characters, and kid-friendly mascots.",
    },
    vintage_retro: {
      ...
      specialInstructions:
        "Include 1950s diner themes, vintage cars, retro patterns, and classic designs.",
    },
    gaming_tech: {
    ...
    holidays: {
      ...
      specialInstructions:
        "Include Christmas, Halloween, Easter, birthdays, and cultural holidays.",
    },
    music_dance: {
      title: "Music & Dance",
    ...
    steampunk: {
      ...
      specialInstructions:
        "Include gears, steam machines, Victorian ladies, and clockwork animals.",
    },
    tribal_ethnic: {
      ...
      specialInstructions:
        "Include tribal masks, cultural symbols, traditional patterns, and ethnic art.",
    },
  };
...
async function renderCategoryWithProgressiveLoading(
  ...
  console.log(
    "Rendering category with progressive loading:",
    categoryKey,
    "Page:",
    currentPage,
    "Sort:",
  ...
  if (window.imageLoadQueue) {
    ...
    if (canceledCount > 0) {
      console.log(
        `[Category Loading] Canceled ${canceledCount} pending image downloads for new category`
      );
    }
  }
...
`;
  ...
  try {
    ...
    if (
      ...
      Object.keys(parsedData.categories).length === 0
    ) {
    ...
    if (categoryKeys.length !== 25) {
      ...
      if (typeof showToast === "function") {
        showToast(
          `Expected 25 categories, but received ${categoryKeys.length}. Some categories may be missing.`,
          "warning"
        );
      }
    }
    ...
    for (const [catKey, category] of Object.entries(parsedData.categories)) {
      if (category.items && Object.keys(category.items).length >= 50) {
        // At least 50 items per category (reduced from 160 for tolerance)
        validCategories++;
      } else {
        ...
        if (typeof showToast === "function") {
          showToast(
            `Category "${category.title || catKey}" has only ${itemCount} items (expected 160).`,
            "warning"
          );
        }
      }
...
document.addEventListener("submit", function (event) {
  ...
  if (event.target.closest("#static-contact")) {
    ...
    alert(
      "Thank you for your message! This is a demo form. In a production environment, this would submit your information to our team."
    );
  }
});
...
function initStyleSelector() {
  ...
  if (styleSelector) {
    ...
    console.log(
      `Style selector initialized with ${Object.keys(COLORING_STYLES).length} styles, current: "${currentColoringStyle}"`
    );
  } else {
